...
def build_select_query_dwt(table_name: str, schema_name: str):
...
def select_watermark_details(schema: str, table_name: str, marking_column: str):
    return SQL(
        """
        SELECT
            {} AS source_system
...
def count_records_after_watermark(table_name: str, schema_name: str,
                                  marking_column: str, highest_watermark: datetime) -> bytes:
    return SQL(
        """
        SELECT COUNT(*) AS egress_volume,
               {} AS table_name,
               {} AS schema_name
        FROM {}.{}
        WHERE {} > {};
